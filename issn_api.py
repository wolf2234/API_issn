...
def parse_issn_data(issn: str, data: dict) -> dict:
    ...
    for item in graph:
        ...
        if id_attr.startswith("http://id.loc.gov/vocabulary/countries"):
            ...
            result['unparsed_keys'].append("resource/ISSN/{issn}#InterveningPublicationEvent")
            unparsed += 1
            ...
            result['unparsed_keys'].append("resource/ISSN/{issn}#PublicationPlace-{placename}-GeoCoordinates")
            unparsed += 1
        elif id_attr.endswith(f"Geocoordinates"):
            result['unparsed_keys'].append('resource/ISSN/{issn}#ReproductionPlace-{placeName}-Geocoordinates')
            unparsed += 1
            ...
            result['unparsed_keys'].append('resource/ISSN/{issn}#LatestPublicationEvent')
            unparsed += 1
...
def parse_issn_resource(item: dict) -> tuple:
    ...
    if '@type' not in item:
        warnings['parse_issn_resource'].append(f"attribute '@type' doesnt exist in {item}")

    if 'url' not in item:
        errors['parse_issn_resource'].append(f"attribute 'url' doesnt exist in {item}")

    if 'name' not in item:
        errors['parse_issn_resource'].append(f"attribute 'name' doesn't exist in {item}")

    if 'otherPhysicalFormat' not in item:
        errors['parse_issn_resource'].append(f"attribute 'otherPhysicalFormat' doesn't exist in {item}")

    if 'title' not in item:
        errors['parse_issn_resource'].append(f"attribute 'title' doesn't exist in {item}")

    if 'isFormatOf' not in item:
        errors['parse_issn_resource'].append(f"attribute 'isFormatOf' doesn't exist in {item}")

    if 'isPartOf' not in item:
        errors['parse_issn_resource'].append(f"attribute 'isPartOf' doesn't exist in {item}")

    if 'identifiedBy' not in item:
        errors['parse_issn_resource'].append(f"attribute 'identifiedBy' doesn't exist in {item}")

    if 'mainTitle' not in item:
        errors['parse_issn_resource'].append(f"attribute 'mainTitle' doesn't exist in {item}")
...
def parse_oraganization(item: dict) -> Union[tuple, None]:
    ...
    if '@type' not in item:
        warnings['parse_oraganization'].append(f"attribute '@type' doesnt exist in {item}")

    if item.get('@type') != "http://schema.org/Organization":
        errors['parse_oraganization'].append(f"attribute '@type' != 'http://schema.org/Organization', {item}")
...
def parse_country_code(item: dict) -> Union[tuple, None]:
    ...
    if '@type' not in item:
        warnings['parse_country_code'].append(f"attribute '@type' doesnt exist in {item}")

    if item.get('@type') != "http://schema.org/PublicationEvent":
        errors['parse_country_code'].append(f"attribute '@type' != 'http://schema.org/PublicationEvent', {item}")

    if 'location' not in item:
        errors['parse_country_code'].append(f"attribute 'location' doesn't exist in {item}")
        return (None, errors, warnings)
...
def parse_issn(issn: str, item: dict) -> Union[dict, tuple]:
    ...
    if item.get('@type') != "http://id.loc.gov/ontologies/bibframe/Issn":
        errors['parse_issn'].append(f"attribute '@type' != 'http://id.loc.gov/ontologies/bibframe/Issn', {item}")
...
def parse_issnL(item: dict) -> Union[dict, tuple]:
    ...
    if item.get('@type') != "http://id.loc.gov/ontologies/bibframe/IssnL":
        errors['parse_issnL'].append(f"attribute '@type' != 'http://id.loc.gov/ontologies/bibframe/IssnL', {item}")
    return (data, errors, warnings)
...
def parse_record(item: dict) -> Union[dict, tuple]:
    ...
    if item.get('@type') != "http://schema.org/CreativeWork":
        errors['parse_record'].append(f"attribute '@type' != 'http://schema.org/CreativeWork', {item}")
...
# def resource_attribute_success(keyname):
#     global cache
#     cache[keyname] = 1 if keyname not in cache else cache[keyname] + 1
